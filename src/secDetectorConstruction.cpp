...
G4VPhysicalVolume* secDetectorConstruction::Construct(void)
{
    ...
    const G4double Plate_Size[3] = {5.*cm, 5*cm, 0.5*cm};
...
//Iron plate
    ...
    new G4PVPlacement(0, G4ThreeVector(), plate_log, "plate_phy", world_log, false, 0, OverLapCheck);
...
//Detector
    ...
    auto sci_log1 = new G4LogicalVolume(sci_geo, sci_mat, "sci_log1");
    auto sci_log2 = new G4LogicalVolume(sci_geo, sci_mat, "sci_log2");
    ...
    new G4PVPlacement(0, G4ThreeVector(0., 0.,  3.*cm), sci_log1, "sci_phy1", world_log, false, 0, OverLapCheck);
    new G4PVPlacement(0, G4ThreeVector(0., 0., -3.*cm), sci_log2, "sci_phy2", world_log, false, 0, OverLapCheck);
//=======================================================================
...
//SiPM
    ...
    //logical volume
    auto pm_log1 = new G4LogicalVolume(pm_geo, pm_mat, "pm_log1");
    auto pm_log2 = new G4LogicalVolume(pm_geo, pm_mat, "pm_log2");
    ...
    new G4PVPlacement(0, 
                      G4ThreeVector(0., 0., 3*cm - Scint_Size[2] - SiPM_Size[2]),
                      pm_log1, 
                      "SiPM1", 
                      world_log, 
                      false, 
...
//Aluminium Foil
    ...
    auto foil_geo1 = new G4Box("foil_body", Foil_Size[0], Foil_Size[1], Foil_Size[2]);
    auto foil_geo2 = new G4SubtractionSolid("foil_box", foil_geo1, sci_geo, 0, G4ThreeVector());
    auto foil_geo  = new G4SubtractionSolid("foil_box_with_hole",
                                                foil_geo2,
                                                pm_geo,
                                                0,
                                                G4ThreeVector(0, 0, -Foil_Size[2] - SiPM_Size[2]));
    ...
    new G4PVPlacement(0, G4ThreeVector(0., 0.,  3.*cm), foil_log1, "Al_foil_phy1", world_log, false, 0, OverLapCheck);
    new G4PVPlacement(0, G4ThreeVector(0., 0., -3.*cm), foil_log2, "Al_foil_phy2", world_log, false, 0, OverLapCheck);
...
{  
    ...
    /*
    G4double SiPMAbsorbEneg[6] = {2.*eV, 3.*eV, 4.*eV, 5.*eV, 5.*eV, 7.*eV};
    G4double SiPMAbsorbLeng[6] = {0.001*mm, 0.001*mm, 0.001*mm, 0.001*mm, 0.001*mm, 0.001*mm};
    auto SiPMTable = new G4MaterialPropertiesTable();
    SiPMTable->AddProperty("ABSLENGTH", SiPMAbsorbEneg, SiPMAbsorbLeng, 6);
    surface->SetMaterialPropertiesTable(SiPMTable);
    */
}
